...
export interface ControlPreflightCheckSpec {
  readonly name: string;
  readonly description: string;
}

export interface CombinedControlPreflightSurface {
  readonly context: ControlProcessContext;
  readonly kind: "source-preflight";
  readonly checks: readonly ControlPreflightCheckSpec[];
  start(args?: {
    host?: string;
    port?: number;
  }): Promise<ControlPreflightRuntime>;
}

export function createDefaultControlPreflightChecks(): readonly ControlPreflightCheckSpec[] {
  return [
    {
      name: "healthz",
...
export async function createCombinedControlPreflightSurface(
  options: ControlTestHarnessOptions = {}
): Promise<CombinedControlPreflightSurface> {
  const harness = await createControlTestHarness(options);

  return {
    context: harness.context,
    kind: "source-preflight",
    checks: createDefaultControlPreflightChecks(),
    start: async (args = {}) => {
